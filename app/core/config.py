...
class  WebhookSettings(BaseSettings):
    ...
    def webhook_alert_emails(self) -> list[str]:
        return [e.strip() for e in self.webhook_alert_emails_raw.split(",") if e.strip()]
    ...
    def validate_config(self) -> None:
        ...
        if not self.webhook_target:
            log.warning("WEBHOOK_TARGET is not set — webhooks will be queued but never delivered.")
