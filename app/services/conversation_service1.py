...
SECTION_OPENING: Dict[str, str] = {
    # ── D1 ────────────────────────────────────────────────────────────────────
    "team_members": (
        "👋 Let's build your **D1 — 8D Investigation Team**.\n\n"
        "A strong 8D team should include cross-functional expertise such as:\n"
        "• Quality representative\n"
        "• Production representative\n"
        "• Engineering or maintenance expert\n"
        "• A team leader coordinating the investigation\n\n"
        "Tell me the team members involved.\n"
        "You can give **a name or email**, and I will retrieve the details "
...
CONV_SYSTEM_PROMPT = """
You are an expert 8D Problem Solving Coach trained in automotive quality methods
(Bosch, IATF 16949, AIAG, QRQC, and structured root cause analysis).

Your role is NOT only to collect information but to actively guide the user
through a rigorous investigation process.

You help a quality engineer complete ONE specific section of an 8D report.

Your behaviour must resemble an experienced quality coach supporting
a real industrial investigation.


COACHING PRINCIPLES

1. Always think critically about the user's answer.
2. If the answer is vague, incomplete, or weak — ask for clarification.
3. Encourage investigation thinking using facts, data, and verification.
4. Do NOT simply accept answers — guide the user toward structured reasoning.
5. Use concrete examples when asking for clarification.
6. Maintain a professional tone similar to a senior quality engineer.


CONVERSATION RULES

1. Ask ONE focused question at a time.
2. Maximum 3 short sentences per response.
3. Acknowledge the user's input briefly before asking the next question.
4. Never overwhelm the user with long explanations.
5. Keep the conversation natural and supportive.


INTERPRETING USER ANSWERS

The user may sometimes answer with short confirmations such as:
...
When the user sends a simple confirmation:

• Do NOT interpret it as a person's name
• Do NOT assume it refers to an employee
• Treat it as confirmation of the previous question
• Continue the conversation normally

Only interpret a message as a team member if it clearly contains:
...
Return the JSON inside this wrapper exactly:

<extracted_fields>
{ ... }
</extracted_fields>

After the JSON block say:

"✅ Section complete! The information has been saved."
...
For team_members ALWAYS use these fields:
...
TEAM MEMBER LOGIC (D1)

For the D1 8D team section:

• Minimum 2 members are required
• One member MUST have function = "team_leader"

Encourage cross-functional representation such as:
...
If the team composition is weak or incomplete,
suggest adding missing roles to strengthen the investigation team.

Example coaching behaviour:

"The investigation team usually includes quality and production.
Do we have someone from production involved in the analysis?"


DIRECTORY AWARENESS

Some team members may already exist in the company employee directory.
If the user mentions a person by name or email, the system may resolve
the employee automatically.
If the user confirms a person already mentioned in the conversation,
do NOT assume it is a new employee.
...
Your tone should resemble a senior quality engineer guiding
...
class ConversationService:
    ...
    # ─────────────────────────────────────────────
    # D1 MEMBER DIRECTORY INTELLIGENCE
    # ─────────────────────────────────────────────

    def _search_members(self, query: str):
        """Search AVO member directory by name or email."""
        directory = MemberDirectory(self.db)
        return directory.search(query)

    def _evaluate_team_balance(self, members):
        """
        ✨ NEW METHOD — Evaluate team composition against Bosch 8D standards.
        
        Returns a list of missing roles that should be added for a balanced team.
        """
        departments = {m.get("department") for m in members}
        functions = {m.get("function") for m in members}

        missing = []

        if "team_leader" not in functions:
            missing.append("a team leader")

        if "quality" not in departments:
            missing.append("a quality representative")

        if "production" not in departments:
            missing.append("a production representative")

        if not {"engineering", "maintenance", "supplier_quality"} & departments:
            missing.append("an engineering or maintenance expert")

        return missing

    def _handle_d1_member_logic(
        self,
        step_id: int,
        section_key: str,
        user_message: str,
        ...
        """
        Intercepts D1 messages to resolve employee names
        before sending to the AI.
        
        🔧 UPDATED — Now accumulates members and provides coaching feedback.
        """
        msg = user_message.strip().lower()
        if msg in {"yes", "no", "ok", "okay", "continue", "correct", "oui", "non"}:
            return None
        members = self._search_members(user_message)
        ...
        if len(members) > 1:
            options = "\n".join(
                f"{i+1}. {m.name} — {m.department} ({m.role})"
                for i, m in enumerate(members)
            )

            reply = (
                "I found multiple employees:\n\n"
                f"{options}\n\n"
            ...
            self._persist_message(
                ...
                reply,
                next_idx,
                meta={"member_candidates": [m.id for m in members]},
                commit=False,
            )

            history.append(self._msg_dict("assistant", reply, next_idx))
        ...
        # 🔧 CHANGE 1: Load existing members instead of replacing
        current = self.get_current_step_data(step_id)
        existing_members = current.get("team_members", [])

        # 🔧 CHANGE 2: Append new member to existing list
        existing_members.append({
            "name": m.name or "",
            "department": (m.department or "other"),
            "function": (m.role or "other"),
        })

        extracted = {"team_members": existing_members}

        self._update_step_data(step_id, extracted, commit=False)

        # 🔧 CHANGE 3: Evaluate team balance and provide coaching
        missing_roles = self._evaluate_team_balance(existing_members)

        if missing_roles:
            reply = (
                f"✅ Added {m.name} — {m.department} ({m.role}).\n\n"
                f"To build a strong 8D investigation team we still need: "
            ...
            reply = (
                f"✅ Added {m.name} — {m.department} ({m.role}).\n\n"
                "Your team composition looks balanced for an 8D investigation.\n"
                "Would you like to add another member or finish the team?"
            )
    ...
    def send_message(
        ...
        if section_key == "team_members":
            result = self._handle_d1_member_logic(
                step_id,
                section_key,
                user_message,
                history,
