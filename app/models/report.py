...
class Report(Base):
    ...
    status = Column(
        ...
        default="draft",
        index=True,
        comment="draft|in_progress|submitted|under_review|approved|rejected",
    )
    created_at = Column(
    ...
    creator = relationship(
        "User", foreign_keys=[created_by], back_populates="created_reports"
    )
    reviewer = relationship(
        "User", foreign_keys=[reviewed_by], back_populates="reviewed_reports"
    )
    steps = relationship(
        "ReportStep", back_populates="report", cascade="all, delete-orphan"
    )
