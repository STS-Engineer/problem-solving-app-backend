...
class User(Base):
    ...
    reported_complaints = relationship(
        "Complaint", foreign_keys="Complaint.reported_by", back_populates="reporter"
    )
    created_reports = relationship(
        "Report", foreign_keys="Report.created_by", back_populates="creator"
    )
    reviewed_reports = relationship(
        "Report", foreign_keys="Report.reviewed_by", back_populates="reviewer"
    )
    completed_steps = relationship(
        "ReportStep", foreign_keys="ReportStep.completed_by", back_populates="completer"
    )
