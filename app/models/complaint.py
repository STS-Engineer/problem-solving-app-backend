...
class Complaint(Base):
    ...
    cqt_email               = Column(String(255), nullable=True,  index=True,
                                     comment="Customer Quality Technician email (external or internal)")
    quality_manager_email   = Column(String(255), nullable=True,  index=True,
                                     comment="AVOCarbon quality manager email")
    plant_manager_email     = Column(String(255), nullable=True,  index=True,
    ...
    audit_logs          = relationship("ComplaintAuditLog", back_populates="complaint",
                                       cascade="all, delete-orphan",
                                       order_by="ComplaintAuditLog.created_at")
    def __repr__(self):
        return f"<Complaint(id={self.id}, ref='{self.reference_number}', status='{self.status}')>"
