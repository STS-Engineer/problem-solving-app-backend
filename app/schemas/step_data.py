...
class D2Data(BaseModel):
    ...
    problem_description: Optional[str] = Field(
        None, description="Description de l'objet/processus et du défaut"
    )
    ...
    observed_situation: Optional[str] = Field(
        None, description="Situation observée (déviation)"
    )
    ...
    is_is_not_factors: List[IsIsNotFactor] = Field(
        default_factory=list, description="Analyse des facteurs IS / IS NOT"
    )
...
class SuspectedPartsRow(BaseModel):
    ...
    location: str = Field(
        ...,
        description="Location (supplier_site|in_transit|production_floor|warehouse|customer_site|others)",
    )
...
class RestartProduction(BaseModel):
    ...
    identification: Optional[str] = Field(
        None, description="Parts and boxes identification"
    )
...
class D3Data(BaseModel):
    ...
    suspected_parts_status: List[SuspectedPartsRow] = Field(
        default_factory=list, description="Suspected parts status by location"
    )
    ...
    containment_responsible: Optional[str] = Field(
        None, description="Containment responsible"
    )
...
class D5Data(BaseModel):
    ...
    corrective_actions_occurrence: List[CorrectiveActionD5] = Field(
        default_factory=list, description="Corrective actions for occurrence"
    )
    corrective_actions_detection: List[CorrectiveActionD5] = Field(
        default_factory=list, description="Corrective actions for detection"
    )
...
class ImplementationMonitoring(BaseModel):
    ...
    monitoring_interval: Optional[str] = Field(
        None, description="Monitoring interval of time"
    )
    pieces_produced: Optional[int] = Field(
        None, description="Number of pieces produced"
    )
...
class D6Data(BaseModel):
    ...
    monitoring: Optional[ImplementationMonitoring] = Field(
        None, description="Section II - Implementation monitoring data"
    )
...
class ClosureSignature(BaseModel):
    ...
    approved_by: Optional[str] = Field(
        None, description="Approved by (Quality Manager)"
    )
...
class D8Data(BaseModel):
    ...
    closure_statement: Optional[str] = Field(
        None, description="Final closure statement"
    )
