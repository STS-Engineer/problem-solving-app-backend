...
def upgrade() -> None:
    ...
    op.drop_constraint("complaints_quality_manager_fkey","complaints", type_="foreignkey")
    ...
    op.add_column("complaints", sa.Column(
        "cqt_email", sa.String(255), nullable=True,
        comment="Customer Quality Technician/Engineer email"
    ))
    op.add_column("complaints", sa.Column(
        "quality_manager_email", sa.String(255), nullable=True,
        comment="AVOCarbon quality manager email"
    ))
    op.add_column("complaints", sa.Column(
        "plant_manager_email", sa.String(255), nullable=True,
        comment="Plant manager email — used for L3/L4 escalation"
    ))
    op.add_column("complaints", sa.Column(
        "approved_by_email", sa.String(255), nullable=True,
    ...
    op.create_index("ix_complaints_quality_manager_email",  "complaints", ["quality_manager_email"])
    op.create_index("ix_complaints_plant_manager_email",    "complaints", ["plant_manager_email"])

    # ── 2. Extend report_steps table ─────────────────────────────────────────

    op.add_column("report_steps", sa.Column(
        "due_date", sa.DateTime, nullable=True,
        comment="SLA-based deadline for this step"
    ))
    op.add_column("report_steps", sa.Column(
        "is_overdue", sa.Boolean, nullable=False,
        server_default="false",
        comment="True once the due_date has passed without completion"
    ))
    op.add_column("report_steps", sa.Column(
        "escalation_count", sa.Integer, nullable=False,
        server_default="0",
        comment="Number of escalation emails sent (0–4)"
    ))
    op.add_column("report_steps", sa.Column(
        "escalation_sent_at", sa.DateTime, nullable=True,
        comment="Timestamp of the most recent escalation email"
    ))
    op.add_column("report_steps", sa.Column(
        "cost", sa.Numeric(12, 2), nullable=True,
    ...
    # Composite indexes
    op.create_index(
        "idx_audit_complaint_step",
        "complaint_audit_log", ["complaint_id", "step_code"]
    )
    op.create_index(
        "idx_audit_complaint_type",
        "complaint_audit_log", ["complaint_id", "event_type"]
    )
    op.create_index(
        "idx_audit_data_gin",
        "complaint_audit_log", ["event_data"],
